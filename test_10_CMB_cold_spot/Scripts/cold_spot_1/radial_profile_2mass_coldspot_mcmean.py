...
Constrói:
  - perfil radial observado em torno do CMB Cold Spot;
...
# Centro do Cold Spot (galáctico) -> já convertido antes para equatorial
# l = 209 deg, b = -57 deg  => RA ~ 48.2999 deg, Dec ~ -20.4373 deg
RA_CS_DEG  = 48.2999
DEC_CS_DEG = -20.4373
...
def main():
    ...
    # Garante RA,Dec,Kmag numéricos (ignorando cabeçalho tipo 'deg', '---', etc.)
    ...
    # Constrói SkyCoord para todas as fontes (para reutilizar nas separations)
    coords = SkyCoord(ra=ra * u.deg, dec=dec * u.deg, frame="icrs")

    # -------------------------------------------------------
    # 3.1 Perfil radial observado em torno do Cold Spot real
    # -------------------------------------------------------
    print("=========================================================")
    print("[INFO] Construindo perfil radial OBSERVADO (Cold Spot real)...")
    ...
    # Ajuste linear no observado (só para referência)
    ...
    # -------------------------------------------------------
    # 3.2 Monte Carlo de centros aleatórios: PERFIL MÉDIO
    # -------------------------------------------------------
    print("=========================================================")
    ...
    # Guardar densidades de cada simulação: shape (N_MC, nbins)
    ...
    for i in range(N_MC):
        # centro aleatório em RA/Dec dentro do retângulo
    ...
    # Podemos também ajustar um slope médio
    ...
    with open(outfile, "w", encoding="utf-8") as f:
        f.write("# Radial density profile: CMB Cold Spot vs average random centers\n")
        f.write(f"# Input catalog: {infile}\n")
        f.write(f"# Cold Spot (RA,Dec) = ({RA_CS_DEG:.4f}, {DEC_CS_DEG:.4f}) deg\n")
        ...
        f.write("#\n")
        f.write("# Linear fit (observed Cold Spot): dens_obs(r) = a_obs + b_obs * r\n")
