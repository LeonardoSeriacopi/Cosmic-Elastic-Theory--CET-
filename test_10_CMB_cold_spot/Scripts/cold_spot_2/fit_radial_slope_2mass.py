...
"""
fit_radial_slope_2mass_coldspot2.py

Ajusta modelos de densidade radial em torno do *segundo* CMB Cold Spot
usando o catálogo 2MASS recortado (2mass.tsv).

- Usa o centro do segundo Cold Spot em coordenadas galácticas
  (l,b) = (208.4349, -55.8545) deg,
  convertido para equatorial (RA,Dec) ≈ (49.3442, -19.7679) deg.
...
- Ajusta um modelo linear: dens(r) = rho0 + a * r
  para:
    ...
    (iii) mid     (12 <= K < 14),
    (iv) faint    (14 <= K < 16).
- Imprime resultados no terminal e salva em
  '2mass_coldspot2_radial_slope_results.txt'.
"""
...
OUTTXT = "2mass_coldspot2_radial_slope_results.txt"

# Centro do SEGUNDO Cold Spot (galáctico) e conversão para equatorial
l_cs = 208.4349
b_cs = -55.8545
...
# (se quiser usar diretamente os valores já calculados, poderia substituir por:)
# RA_CS  = 49.3442
# DEC_CS = -19.7679
...
def weighted_linear_fit(x, y, yerr):
    ...
    # coeficientes
    a = (Wxx * Wy - Wx * Wxy) / denom   # intercepto (rho0)
...
def main():
    ...
    big_sep(57, "=")
    print("[INFO] Calculando distâncias angulares até o *segundo* Cold Spot...")
    ...
    print(f"  Segundo Cold Spot (galático): (l,b) = ({l_cs}, {b_cs}) deg")
    print(f"  Segundo Cold Spot (ICRS):     (RA,Dec) = ({RA_CS:.4f}, {DEC_CS:.4f}) deg")
    print(f"  Radial bins: {RADIAL_BINS} deg")
    big_sep(57, "=")

    with open(OUTTXT, "w", encoding="utf-8") as f:
        f.write("Radial slope fits for 2MASS around SECOND CMB Cold Spot\n")
        f.write(f"Second Cold Spot (l,b) = ({l_cs}, {b_cs}) deg\n")
        f.write(f"Second Cold Spot (RA,Dec) = ({RA_CS:.6f}, {DEC_CS:.6f}) deg\n")
        ...
        for label, (kmin, kmax) in MAG_BINS.items():
            ...
            print(f"    rho0   = {a: .3f} ± {sigma_a: .3f} sources/deg^2")
            print(f"    slope  = {b: .3f} ± {sigma_b: .3f} sources/deg^2/deg")
