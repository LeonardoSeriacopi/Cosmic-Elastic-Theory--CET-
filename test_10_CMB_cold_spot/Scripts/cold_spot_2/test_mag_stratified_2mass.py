...
"""
Testa se o déficit/excesso de fontes ao redor do SEGUNDO Cold Spot
depende da magnitude K (2MASS).

Entrada:
    - 2mass.tsv  (catálogo recortado que você já baixou do VizieR)

Saída:
    - mag_stratified_2mass_cs2_results.txt
"""
...
# Centro do SEGUNDO Cold Spot (coordenadas galácticas)
# (era L_CS=209.0, B_CS=-57.0 no primeiro teste)
L_CS = 208.4349   # deg
B_CS = -55.8545   # deg
...
def load_2mass_catalog(infile):
    ...
    print(f"[INFO] Linhas com RA/Dec/Kmag válidos após limpeza: {len(df)}")
    return df


def compute_theta_deg(df, l_cs, b_cs):
    """
    Calcula a distância angular (em graus) de cada fonte
    ao centro do Cold Spot, dado em (l,b) galáctico.
    """
    # Centro do Cold Spot em galáctico
    cs_gal = SkyCoord(l=l_cs * u.deg, b=b_cs * u.deg, frame="galactic")
    # Catálogo em equatorial
...
def main():
    ...
    # 2) Calcular theta_deg para todas as fontes (segundo Cold Spot)
    theta_deg = compute_theta_deg(df, L_CS, B_CS)

    # 3) Arquivo de saída
    outname = "mag_stratified_2mass_cs2_results.txt"
    f = open(outname, "w", encoding="utf-8")

    f.write("Mag-stratified radial density test around the SECOND CMB Cold Spot\n")
    ...
    print("=========================================================")
    print("[INFO] Iniciando testes por faixa de magnitude K (SEGUNDO Cold Spot)")
    print("=========================================================")
    ...
    for mmin, mmax, label in MAG_BINS:
        ...
        # Ajuste de densidade constante nessa faixa de magnitude
