...
"""
plot_radial_profile_2mass_coldspot2.py

Plots:
 - the observed radial number–density profile around the *second* CMB Cold Spot
 - the MEAN radial profile from random centres
with error bars.

Requires the file produced by radial_profile_2mass_coldspot_mcmean.py
(or its analogue for the second Cold Spot), e.g.:
    resultado_radial_profile_2mass_coldspot_mcmean_2.txt
"""

import numpy as np
import matplotlib.pyplot as plt

infile = "resultado_radial_profile_2mass_coldspot_mcmean_2.txt"
...
def main():
    ...
    plt.errorbar(
        ...
        fmt="o-", capsize=3, label="Second Cold Spot (observed)",
        color="tab:blue"
    )
    ...
    plt.xlabel(r"Radius $\theta$ around second CMB Cold Spot [deg]")
    plt.ylabel(r"2MASS number density [deg$^{-2}$]")
    plt.title("Radial Number–Density Profile: Second Cold Spot vs Random Sky")
    plt.grid(alpha=0.3)
    plt.legend()

    plt.tight_layout()
    plt.savefig("fig_coldspot2_radial_profile.png", dpi=300)
    ...
    plt.title("Second Cold Spot Density Excess Relative to Random Sky")
    plt.grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig("fig_coldspot2_density_excess.png", dpi=300)

    print("[OK] Figures saved as:")
    print("     fig_coldspot2_radial_profile.png")
    print("     fig_coldspot2_density_excess.png")
