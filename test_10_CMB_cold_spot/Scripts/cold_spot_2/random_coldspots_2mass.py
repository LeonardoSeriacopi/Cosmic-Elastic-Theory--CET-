...
"""
Random Cold Spots test em torno do SEGUNDO CMB Cold Spot usando 2MASS recortado.

- Usa o catálogo 2MASS recortado (2mass.tsv), centrado em um cone de 10° ao redor
  do SEGUNDO Cold Spot.
- Calcula o perfil radial observado em torno desse Cold Spot.
- Gera centros aleatórios dentro do mesmo recorte (amostrando posições de galáxias).
...
Saída: estatísticas no terminal e arquivo de resumo:
    random_coldspots_2mass_cs2_results.txt
"""
...
# SEGUNDO Cold Spot
# Galáctico (para referência): l = 208.4349 deg, b = -55.8545 deg
...
def ang_sep_deg(ra1_deg, dec1_deg, ra2_deg, dec2_deg):
    """
    Distância angular em graus entre dois pontos na esfera (RA, Dec).
...
def build_radial_profile(ra_deg, dec_deg, kmag, ra0_deg, dec0_deg,
    ...
    # Considera apenas até o último raio
...
def fit_linear_density(r_centers, densities, sigmas):
    ...
    # BUG CORRIGIDO: era Svr2, agora Swr2
    Delta = Sw * Swr2 - Swr**2
    if Delta == 0:
        return (np.nan, np.nan, np.nan, np.nan, np.nan, 0)
...
def main():
    print("=========================================================")
    print(f"[INFO] SEGUNDO Cold Spot:")
    print(f"  RA  = {RA_CS:.4f} deg")
    print(f"  Dec = {DEC_CS:.4f} deg")
    print("---------------------------------------------------------")
    ...
    print(f"[INFO] θ range do recorte vs Cold Spot: {theta_cs.min():.2f} – {theta_cs.max():.2f} deg")
    print("=========================================================")
    ...
    outfile = "random_coldspots_2mass_cs2_results.txt"
    with open(outfile, "w", encoding="utf-8") as f:
        f.write("Random Cold Spots test around the SECOND CMB Cold Spot (2MASS cutout)\n")
        f.write("-------------------------------------------------------------------\n\n")
