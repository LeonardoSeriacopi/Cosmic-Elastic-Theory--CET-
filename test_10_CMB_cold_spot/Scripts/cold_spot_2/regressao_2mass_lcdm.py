...
OUTFILE = "regressao_2mass_lcdm_cs2_results.txt"

# Centro do *segundo* Cold Spot em coordenadas galácticas
# (posição que você usou no VizieR)
L_CS = 208.4349  # graus
B_CS = -55.8545  # graus
...
def main():
    ...
    # Distância angular até o Cold Spot 2
    ...
    for i in range(len(r_mid)):
        ...
        print(
            f" {r_in:4.1f}  {r_out:4.1f}  {counts[i]:6d}  "
            f"{areas[i: i+1][0]:10.2f}  {density[i]:10.3f}  {sigma[i]:8.3f}"
        )
    ...
    with open(OUTFILE, "w", encoding="utf-8") as f:
        f.write("2MASS radial density around SECOND CMB Cold Spot\n")
        f.write("Input catalog : {}\n".format(INFILE))
        f.write("Cold Spot 2 (l,b) = ({:.4f}, {:.4f}) deg\n".format(L_CS, B_CS))
        ...
        for i in range(len(r_mid)):
            ...
            r_out = RADIAL_BINS[i+1]
            f.write(
                f"{r_in:6.2f}  {r_out:6.2f}  {counts[i]:7d}  "
                f"{areas[i]:12.3f}  {density[i]:12.3f}  {sigma[i]:8.3f}\n"
            )
